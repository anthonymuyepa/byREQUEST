...
' ---------- CONFIG ----------
Dim INI_PATH
INI_PATH = "S:\CarolinaTrust\byREQUEST\Templates\GL_Income.ini"
...
Sub InsertMappedRow_GL(ByVal csvLine, ByVal lineOrder)
    ...
    Dim reportDateVal
    Dim glAcc, glName
    Dim mthToDate, mthGrossPct
    Dim qtrToDate, qtrGrossPct
    Dim yearToDate, yearGrossPct
    Dim sql, cmd, rowsAff

    f = SplitCSV_Delimited(csvLine, gDelimiter)

    If Not IsArray(f) Then
        LogLine "SKIP: Could not split CSV row"
        LogLine "Line: " & Left(csvLine, 300)
        Exit Sub
    End If
    ...
    ' 9 Gross%
    If colCount < 10 Then
        LogLine "SKIP: Not enough CSV fields. Need 10. Got " & colCount
        LogLine "Line: " & Left(csvLine, 300)
        Exit Sub
    End If
    ...
    If IsNull(reportDateVal) Then
        LogLine "SKIP: Invalid Report_Date: " & SafeField(f, colCount, 0)
        LogLine "Line: " & Left(csvLine, 300)
        Exit Sub
    End If

    glAcc       = SafeField(f, colCount, 2)
    glName      = SafeField(f, colCount, 3)
    mthToDate   = SafeField(f, colCount, 4)
    mthGrossPct = SafeField(f, colCount, 5)
    qtrToDate   = SafeField(f, colCount, 6)
    qtrGrossPct = SafeField(f, colCount, 7)
    yearToDate  = SafeField(f, colCount, 8)
    yearGrossPct= SafeField(f, colCount, 9)

    If Len(Trim(glAcc)) = 0 Then
        LogLine "SKIP: GLAccount blank"
        LogLine "Line: " & Left(csvLine, 300)
        Exit Sub
    End If

    sql = "INSERT INTO " & gTable2Part & " (" & _
          "[Report_Date],[GLAccount],[GLAccountName]," & _
          "[MthToDate],[MthGrossPct],[QtrToDate],[QtrGrossPct],[YearToDate],[YearGrossPct]," & _
    ...
    With cmd
        ...
        AddParameterString cmd, 30,  mthToDate
        AddParameterString cmd, 30,  mthGrossPct
        AddParameterString cmd, 30,  qtrToDate
        AddParameterString cmd, 30,  qtrGrossPct
        AddParameterString cmd, 30,  yearToDate
        AddParameterString cmd, 30,  yearGrossPct
...
Function EnsureStagingTable_GL()
    ...
    sql = "IF OBJECT_ID(N'" & SqlQuote(gObjIdName) & "', N'U') IS NULL " & _
          ...
          "  MthToDate NVARCHAR(30) NULL," & _
          "  MthGrossPct NVARCHAR(30) NULL," & _
          "  QtrToDate NVARCHAR(30) NULL," & _
          "  QtrGrossPct NVARCHAR(30) NULL," & _
          "  YearToDate NVARCHAR(30) NULL," & _
          "  YearGrossPct NVARCHAR(30) NULL," & _
