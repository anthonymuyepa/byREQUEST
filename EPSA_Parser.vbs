...
Dim header, EPSA, PCTL, LCTL, ODFI_ID, SETTLDTE, ACH_Batch_ID, Post_Time,Post_Date,Post_Seq, File_Post_Seq, ACH_Tran_Code, Report, companyName
header = "Report Post_Date   Post_Time Settlement_Date Company_Name                  ACH_Batch_ID PCTL   LCTL   Post_Seq File_Post_Seq TransCode Description                  ST TYPE     EFF DATE         AMOUNT   CHECK#  SDC#      MEMBER# S/L NAME                                 FEE   RR   TC  RDFI IDENT  DFI ACCOUNT#      INDIVIDUAL ID#         INDIVIDUAL NAME        DD TRACE#          MESSAGE                               ODFI_ID"
...
Sub StartDoc()
    ...
    For i = 1 to 14
		...
		If instr(srec, "CO NAME:") > 0 Then
			...
			End If
		End If
		SpoolFile.User2 = Trim(companyName) 
		SpoolFile.User1 = Spoolfile.name
        ...
        If instr(srec, "SETTL DTE:") > 0 Then
            Dim settlPos, odfiPos
            settlPos = instr(srec, "SETTL DTE:") + 10
            SETTLDTE = Trim(mid(srec, settlPos, 10))
							
			odfiPos = instr(srec, "ORIG DFI ID:") + 12
            ODFI_ID = Trim(mid(srec, odfiPos, 9))
	...
	If UBound(dateTime) > 0 Then
		Post_Date = dateTime(0)  ' "12-15-25"
		Post_Time = dateTime(1) ' "02:20:11"
		Post_Date = Right("0" & Month(Post_Date), 2) & "/" & _
                Right("0" & Day(Post_Date), 2) & "/" & _
                Year(Post_Date)
...
Sub ProcessLine()
  ...
  ' NEW: update rolling context if we hit repeated headers inside the body
  If UpdateContextFromLine(line) Then
      Exit Sub
  End If
  ...
  If gPendingLine <> "" Then
    If IsContinuationLine(line) Then
      ...
      fout.WriteLine Report &" " & Post_Date &"  " & Post_Time & "  " & SETTLDTE &"      " & companyName & "        " & ACH_Batch_ID & PCTL &  LCTL   & Post_Seq & "       " & File_Post_Seq & "             " & enrichType & " " & enrichDesc & " " & gPendingLine & " " & line & "                                  " & ODFI_ID 
...
'==========================
' Context updates while reading body (ProcessLine)
'==========================
Private Function PadRight(ByVal s, ByVal n)
    s = Trim(s & "")
    If Len(s) >= n Then
        PadRight = Left(s, n)
    Else
...
Private Function ExtractBetween(ByVal s, ByVal startToken, ByVal endToken)
    Dim p1, p2
...
' Returns True if this line updated any of the context fields (companyName / SETTLDTE / ACH_Batch_ID)
Private Function UpdateContextFromLine(ByVal s)
    UpdateContextFromLine = False

    ' CO NAME:
    If InStr(1, s, "CO NAME:", vbTextCompare) > 0 Then
        Dim nm
        nm = ExtractBetween(s, "CO NAME:", " ID:")
        companyName = PadRight(nm, 22)
        SpoolFile.User2 = Trim(companyName)
        UpdateContextFromLine = True
        Exit Function
    End If

    ' SETTL DTE:
    If InStr(1, s, "SETTL DTE:", vbTextCompare) > 0 Then
        Dim settlPos, tmp, odfiPos
        settlPos = InStr(1, s, "SETTL DTE:", vbTextCompare) + 10
        tmp = Trim(Mid(s, settlPos, 10))
        SETTLDTE = tmp  ' keep as-is (no padding previously)
		
					
			odfiPos = instr(s, "ORIG DFI ID:") + 12
            ODFI_ID = Trim(mid(s, odfiPos, 9))
    ...
    ' ACH BTCH#:
    If InStr(1, s, "ACH BTCH#:", vbTextCompare) > 0 Then
        Dim achPos, bid
        achPos = InStr(1, s, "ACH BTCH#:", vbTextCompare) + 10
        bid = Trim(Mid(s, achPos, 7))
        ACH_Batch_ID = PadRight(bid, 13)
        UpdateContextFromLine = True
        Exit Function
    End If
End Function
