...
Sub DisplayVendorAvailabilityKPIs(cn, qStart, qEnd)
	...
	Fout.WriteLine PadRight("Vendors Meeting Goal", 30) & _
               PadRight(vendorsMeetingGoal & "/" & vendorCount, 15) & _
               PadRight(FormatNumber((vendorsMeetingGoal / vendorCount) * 100, 1) & "%", 15)
			   
	Fout.WriteLine PadRight("Vendors at/above Yellow",30) & _
               PadRight(vendorsAtYellow & "/" & vendorCount,15) & _
               PadRight(FormatNumber((vendorsAtYellow/vendorCount)*100,1) & "%",15)
			   
    Fout.WriteLine PadRight("Perfect Availability Vendors", 30) & _
               PadRight(perfectAvailabilityVendors & "/" & vendorCount, 15) & _
               PadRight(FormatNumber((perfectAvailabilityVendors / vendorCount) * 100, 1) & "%", 15)

    Fout.WriteLine PadRight("Max Single Vendor Downtime", 30) & FormatNumber(maxDowntimeHours, 2) & " hours"
	Fout.WriteLine PadRight("Severity Distribution", 30) & _
               PadRight("Critical: " & criticalIncidents, 15) & _
               PadRight("High: " & highIncidents, 15)
    Fout.WriteLine PadRight("Total Downtime - Current", 30) & FormatNumber(totalDownMin / 60, 2) & " hours"
    Fout.WriteLine PadRight("Total Incidents -Current", 30) & totalIncidents
