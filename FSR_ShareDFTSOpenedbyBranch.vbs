...
Dim folderPath, connString, tablename
	...
	tablename = "FSR_ShareDFTSOpenedbyBranch"
    ...
    For Each branchFolder In fso.GetFolder(folderPath).SubFolders
        ...
        If fso.FolderExists(subFolderPath) Then
            ...
            For Each file In fso.GetFolder(subFolderPath).Files
				...
				If InStr(LCase(file.Name), "share_dfts_opened_by_branch-") > 0 And _
...
Function SummarizeShareDFTSOpened(ws, startRow, endRow)
    Dim dict: Set dict = CreateObject("Scripting.Dictionary")
    Dim r, empID, balance
    For r = startRow To endRow
        empID = CleanString(ws.Cells(r, "F").Value)
        ...
        Else
            If Not dict.Exists(empID) Then
                ...
                s.Add "NewDrafts", 0
                s.Add "SumNewDrafts", 0
                dict.Add empID, s
            End If
            Set s = dict(empID)
            s("NewDrafts") = s("NewDrafts") + 1
            s("SumNewDrafts") = s("SumNewDrafts") + balance
        End If
    Next
    Set SummarizeShareDFTSOpened = dict
End Function
...
Sub InsertStats(c, statsDict, dateVal, moName, br)
    ...
    For Each empID In statsDict.Keys
        ...
        ElseIf rs.EOF Then
            ' === Insert if not found ===
            sql = "INSERT INTO " & tablename & " (DateCol, Month, Branch, EmpID, NewDrafts) VALUES (" & _
                  "'" & dateVal & "', '" & moName & "', '" & Replace(br, "'", "''") & "', '" & Replace(empID, "'", "''") & "', '" & s("NewDrafts") & "')"
