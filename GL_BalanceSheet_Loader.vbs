...
'   Report_Date  Seq     GL Account  Name                                                               Amount
'
' Example:
'   02/28/2026   331395  710201      M&T BANK                                                           349013.87
'
' Notes:
' - Seq is ignored
' - Name is taken as everything between GLAccount and Amount
' - Amount is stored raw text so SQL can normalize later if needed
' ============================================================
...
' ---------- CONFIG ----------
Dim INI_PATH
INI_PATH = "S:\CarolinaTrust\byREQUEST\Templates\GL_BalanceSheet.ini"
...
Sub ProcessLine()
    ...
    ' Skip the first header row if present
    ...
    ' Skip divider / noise rows
    If InStr(1, line, "Report_Date", vbTextCompare) > 0 Then Exit Sub
...
Sub InsertMappedRow(ByVal srcLine, ByVal lineOrder)
	...
	Dim reportDateVal, glAccount, glName, amountRaw
    Dim sql, cmd, rowsAff
    ...
    If Not ParseGLBalanceSheetLine(srcLine, reportDateVal, glAccount, glName, amountRaw) Then
        LogLine "SKIP: Could not parse line"
        LogLine "Line: " & Left(srcLine, 220)
        Exit Sub
    End If

	sql = "INSERT INTO " & gTable2Part & " (" & _
		  "Report_Date, GLAccount, GLAccountName, Amount, LineOrder, SourceFile" & _
		  ") VALUES (?,?,?,?,?,?)"
    ...
    With cmd
        ...
        AddParameterString cmd, 20,  glAccount
        AddParameterString cmd, 200, glName
        AddParameterString cmd, 30,  amountRaw
...
Function EnsureStagingTable()
	...
	sql = "IF OBJECT_ID(N'" & SqlQuote(gObjIdName) & "', N'U') IS NULL " & _
		  ...
		  "  GLAccountName NVARCHAR(200) NULL," & _
    ...
    If Not EnsureIndex("IX_GLBS_RunDate", "Report_Date") Then Exit Function
    If Not EnsureIndex("IX_GLBS_GLAccount", "GLAccount") Then Exit Function
    If Not EnsureIndex("IX_GLBS_SourceFile", "SourceFile") Then Exit Function
...
Function ParseGLBalanceSheetLine(ByVal srcLine, ByRef outReportDate, ByRef outGLAccount, ByRef outGLName, ByRef outAmount)
    On Error Resume Next

    Dim arr

    ParseGLBalanceSheetLine = False
    outReportDate = Null
    outGLAccount = ""
    outGLName = ""
    outAmount = ""
    ...
    If UBound(arr) < 4 Then Exit Function

    outReportDate = ParseDateFlexibleToVariantDate(arr(0))
    If IsNull(outReportDate) Then Exit Function

    outGLAccount = arr(2)
    outGLName = arr(3)
    outAmount = arr(4)

    ParseGLBalanceSheetLine = True
    On Error GoTo 0
End Function
