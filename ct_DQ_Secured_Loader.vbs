...
' ---------- CONFIG ----------
Dim INI_PATH
INI_PATH = "S:\CarolinaTrust\byREQUEST\Templates\DQ_Secured.ini"
...
Sub StartDoc()
    ...
    LogLine "Delinquent Consumer Secured Import Started: " & Now()
...
Sub InsertMappedRow(ByVal srcLine, ByVal lineOrder)
    ...
    Dim periodDate, accountNo, idVal, nameVal, typeVal, descVal
    ...
    If Not ParseDelinquentRow(srcLine, periodDate, accountNo, idVal, nameVal, typeVal, descVal, lastPaid, dueDate, daysDQ, balanceRaw, codeVal) Then
        LogLine "SKIP: Could not parse line"
        LogLine "Line: " & Left(srcLine, 300)
        Exit Sub
    End If

    ' First valid row determines the period to reload
    ...
    sql = "INSERT INTO " & gTable2Part & " (" & _
          "PeriodDate, AccountNo, IDValue, FullName, LoanType, Description, LastPaid, DueDate, DaysDQ, BalanceRaw, CodeValue, LineOrder, SourceFile" & _
          ") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
    ...
    With cmd
        ...
        AddParameterString cmd, 20,  idVal
...
Function ParseDelinquentRow(ByVal srcLine, _
    ByRef outPeriodDate, ByRef outAccountNo, ByRef outID, ByRef outName, ByRef outType, _
    ...
    If UBound(arr) < 10 Then Exit Function
    ...
    outID         = IntelligentTrim(arr(2))
    outName       = IntelligentTrim(arr(3))
    outType       = IntelligentTrim(arr(4))
    outDesc       = IntelligentTrim(arr(5))
    outLastPaid   = IntelligentTrim(arr(6))
    outDueDate    = IntelligentTrim(arr(7))
    outDaysDQ     = IntelligentTrim(arr(8))
    outBalanceRaw = IntelligentTrim(arr(9))
    outCode       = IntelligentTrim(arr(10))
...
Function EnsureStagingTable()
    ...
    If Not EnsureIndex("IX_DCS_PeriodDate", "PeriodDate") Then Exit Function
    If Not EnsureIndex("IX_DCS_AccountNo", "AccountNo") Then Exit Function
    If Not EnsureIndex("IX_DCS_SourceFile", "SourceFile") Then Exit Function
...
Function ParseIntegerOrNull(ByVal s)
    On Error Resume Next
    ParseIntegerOrNull = Null
...
Sub AddParameterIntOrNull(cmdObj, intVal)
    Dim param
    On Error Resume Next
    Set param = cmdObj.CreateParameter("", adInteger, adParamInput, , intVal)
    cmdObj.Parameters.Append param
...
Sub CloseDoc()
    ...
    LogLine "Delinquent Consumer Secured Import Completed: " & Now()
    LogLine "========================================="
...
Function DeleteOldPeriodRows(ByVal anchorPeriodDate)
    ...
    ' Keep current month and prior 2 months
    ' Delete anything older than the first day of the month 3 months back
