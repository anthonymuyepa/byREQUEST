...
Sub InsertMappedRow(ByVal csvLine)
	...
	sql = "INSERT INTO " & gTable2Part & " (" & _
		  ...
		  "[Status],[Tran_Type],[Tran_Code],[Effective_Date],[Amount],[Member_Nbr],[Mbr_Name],[Fee]," & _
		  "[ACH_Ind_Name],[DD],[Trace_Nbr],[Description],[Type],[Post_Seq],[File_Post_Seq]," & _
		  "[Source_Name],[Check_Number],[SDC_Number],[S_L],[RR],[RDFI_ID],[DFI_ACCOUNT],[Individual_ID],[MESSAGE]," & _
		  "[EPSA_File]" & _
		  ") VALUES (" & _
		  "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?" & _
		  ")"
	...
	With cmd
		...
		' 32 parameters in exact order:
		AddParameterString cmd, 10,  f(0)         ' Report
		AddParameterDate   cmd,       postDateVal ' Post_Date
		...
		AddParameterString cmd, 100, f(4)         ' Source_Name       (still from Company_Name)
		...
		AddParameterString cmd, 20,  f(25)        ' DFI_ACCOUNT       ← only here now
...
Function EnsureStagingTable()
    ...
    sql = "IF OBJECT_ID(N'" & SqlQuote(gObjIdName) & "', N'U') IS NULL " & _
          ...
          " [DFI_ACCOUNT] NVARCHAR(20) NULL," & _
...
Sub CloseDoc
    ...
    LogLine "File processed: " & gEPSA_File
    LogLine "Rows successfully inserted: " & gRowsInserted
