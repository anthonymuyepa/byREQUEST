...
Sub StartDoc
    ...
    Dim endDateObj, priorDateObj, commonReportDate
	...
	commonReportDate = ResolveCommonReportDate(conn)
	
	If IsNull(commonReportDate) Then
		LogLine "ERROR: Could not resolve a common Report_Date across GLBudgetReport, GLIncomeStatement, and GLBalanceSheet."
		CleanupObjects conn, rs, Nothing
		Exit Sub
	End If

	endDateObj = CDate(commonReportDate)
	...
	fout.WriteLine "Income Statement Budget Variance " & Fmt(endDateObj, "mmm d, yyyy")
    WriteHeaderRow
	...
	fout.WriteLine "Balance Sheet Budget Variance " & Fmt(endDateObj, "mmm d, yyyy")
...
Function BuildIncomeStatementVarianceSql()
	...
	sql = sql & "    ActualPriorMTD = MAX(" & nPrior & "), " & vbCrLf
	sql = sql & "    LineOrderPrior = MIN(LineOrder) " & vbCrLf
	sql = sql & "  FROM dbo.GLIncomeStatement " & vbCrLf
	sql = sql & "  WHERE Report_Date = @Prior " & vbCrLf
	...
	sql = sql & "  WHERE Report_Date = @AsOf AND UPPER(ISNULL(Prd,'')) = 'MTD' " & vbCrLf
	sql = sql & "  GROUP BY GLAccount " & vbCrLf
	sql = sql & "), " & vbCrLf
	...
	sql = sql & "  FROM dbo.GLBudgetReport " & vbCrLf
	sql = sql & "  WHERE Report_Date = @AsOf AND UPPER(ISNULL(Prd,'')) = 'YTD' " & vbCrLf
	sql = sql & "  GROUP BY GLAccount " & vbCrLf
	sql = sql & ") " & vbCrLf

	sql = sql & "SELECT " & vbCrLf
	sql = sql & "  p.LineOrderPrior AS [LineOrder], " & vbCrLf
	sql = sql & "  p.GLAccount AS [GLAccount], " & vbCrLf
	sql = sql & "  p.GLAccountName AS [GLAccountName], " & vbCrLf
	...
	sql = sql & "FROM prior p " & vbCrLf
	sql = sql & "LEFT JOIN mtd m ON m.GLAccount = p.GLAccount " & vbCrLf
	sql = sql & "LEFT JOIN ytd y ON y.GLAccount = p.GLAccount " & vbCrLf
	sql = sql & "ORDER BY " & vbCrLf
	sql = sql & "  CASE WHEN p.LineOrderPrior IS NULL THEN 1 ELSE 0 END, " & vbCrLf
	sql = sql & "  p.LineOrderPrior, " & vbCrLf
	sql = sql & "  TRY_CONVERT(bigint, LEFT(p.GLAccount, CHARINDEX('-', p.GLAccount + '-') - 1)), " & vbCrLf
	sql = sql & "  p.GLAccount;"
...
Function BuildBalanceSheetVarianceSql()
    ...
    nActual = SqlNormalizeNumber("Actual")
    nBudget = SqlNormalizeNumber("Budget")
    nPrior  = SqlNormalizeNumber("Amount")
...
Sub WriteDataRow(ByRef r)
    fout.WriteLine _
        ...
        Pad(FmtMoney(r("BudgetMTD")), 20) & _
        Pad(FmtMoney(r("DiffMTD")), 20) & _
        Pad(FmtPct100(r("PctDiffMTD")), 10) & _
        Pad(FmtMoney(r("ActualYTD")), 20) & _
        Pad(FmtMoney(r("BudgetYTD")), 20) & _
        Pad(FmtMoney(r("DiffYTD")), 20) & _
        Pad(FmtPct100(r("PctDiffYTD")), 20) & _
        Pad(FmtMoney(r("ActualPriorMTD")), 20) & _
        Pad(FmtMoney(r("DiffPriorMTD")), 20) & _
        Pad(FmtPct100(r("PctDiffPriorMTD")), 20)
End Sub
' =========================================================
' Generic helpers
' =========================================================
...
Function ResolveMonthlyAsOfDate(ByRef c, ByVal monthStart, ByVal monthEnd)
    ...
    sql = sql & "WITH d AS ( "
    sql = sql & "    SELECT Report_Date AS Dt "
    sql = sql & "    FROM dbo.GLBudgetReport "
    sql = sql & "    WHERE Report_Date >= @MonthStart AND Report_Date <= @MonthEnd "
    sql = sql & "    UNION ALL "
    sql = sql & "    SELECT Report_Date AS Dt "
