...
Const TEMPLATE_SHEET_NAME = "FSRTemplate"
Const TOTALS_SHEET_NAME   = "Branch Totals"

' Return a unique temp path for the working copy
Function GetTempCopyPath(branch)
    Dim tmp, base, p, n
    tmp  = fso.GetSpecialFolder(2)   ' 2 = TemporaryFolder
    base = "FSRTemplate_" & Year(Date) & "_" & Replace(Trim(branch), " ", "_")
    n = 0
    Do
        If n = 0 Then
            p = fso.BuildPath(tmp, base & ".xlsx")
        Else
            p = fso.BuildPath(tmp, base & "~" & n & ".xlsx")
        End If
        n = n + 1
...
' === NEW CONFIG ===
Dim templatePath
templatePath = "S:\EastTexas\FSRTemplate.xlsx" ' <-- Template sheet named "FSRTemplate"

' --- Helpers ---
Function SheetExists(wb, sName)
    Dim s : On Error Resume Next
    Set s = wb.Sheets(sName)
    SheetExists = Not s Is Nothing
    Set s = Nothing
    On Error GoTo 0
End Function

Function SafeSheetName(baseName, wb)
    Dim nm, n, maxLen
    maxLen = 31
    nm = baseName
    ' strip invalid chars: \ / ? * [ ]
    ...
    nm = Replace(nm, "[", "")
    nm = Replace(nm, "]", "")
    If Len(nm) = 0 Then nm = "Sheet"
    If Len(nm) > maxLen Then nm = Left(nm, maxLen)

    n = 1
    Do While SheetExists(wb, nm)
        nm = Left(baseName, maxLen - Len("~" & n)) & "~" & n
        n = n + 1
    Loop
    SafeSheetName = nm
End Function

Function GetBranchesThisYear()
    Dim rs, sql, arr(), i
    Dim currentYear
    Dim c : Set c = OpenConnection()
    If c Is Nothing Then Exit Function

    currentYear = Year(Date)
    
    sql = "SELECT DISTINCT Branch FROM dbo.FSR_DetailUserBranch " & _
          "WHERE LEFT(DateCol,4) = '" & currentYear & "' AND ISNULL(Branch,'') <> '' ORDER BY Branch"
    
    ' Show message box with the year and SQL query
    'MsgBox "Year used: " & currentYear & vbCrLf & vbCrLf & "SQL Query:" & vbCrLf & sql, vbInformation, "Debug Info"

    On Error Resume Next
    Set rs = c.Execute(sql)
    If Err.Number <> 0 Then
        LogError "GetBranchesThisYear failed: " & Err.Description
        Err.Clear
        Set rs = Nothing
    ...
    If Not (rs Is Nothing) Then
        ...
        Do Until rs.EOF
            i = i + 1
            If i = 0 Then ReDim arr(0) Else ReDim Preserve arr(i)
            arr(i) = Trim(rs(0).Value)
            rs.MoveNext
        Loop
        rs.Close : Set rs = Nothing
    End If
    c.Close : Set c = Nothing
    GetBranchesThisYear = arr
End Function

Function GetEmployeeNamesForBranchThisYear(branch)
    Dim rs, sql, arr(), i
    Dim currentYear
    Dim c : Set c = OpenConnection()
    If c Is Nothing Then 
        GetEmployeeNamesForBranchThisYear = Array()
        Exit Function
    End If
	...
	sql = "SELECT DISTINCT d.FSR_Name AS EmpName, " & _
		  " d.FSR_ID AS EmpID " &  _ 
		  "FROM [byREQUEST].[dbo].[FSR_TotalAccountsAssisted] d " & _
		  "WHERE d.Branch = '" & SqlSafe(branch) & "' " & _
		  "  AND LEFT(d.DateCol, 4) = '" & currentYear & "' " & _
		  "  AND ISNULL(LTRIM(RTRIM(d.FSR_Name)), '') <> '' " & _
		  "ORDER BY d.FSR_Name"
    ...
    Set rs = c.Execute(sql)
    If Err.Number <> 0 Then
        LogError "GetEmployeeNamesForBranchThisYear failed: " & Err.Description
        Err.Clear
        Set rs = Nothing
        c.Close : Set c = Nothing
        GetEmployeeNamesForBranchThisYear = Array()
    ...
    If Not (rs Is Nothing) Then
        ...
        Do Until rs.EOF
            ...
            End If
            ' Store both EmpName and EmpID in the array
            arr(i) = Trim(rs("EmpName").Value) & "|" & Trim(rs("EmpID").Value)
            rs.MoveNext
        Loop
        rs.Close : Set rs = Nothing
    End If
    ...
    GetEmployeeNamesForBranchThisYear = arr
End Function

' Helper function to print all returned records
Sub PrintReturnedRecords(arr, branch, currentYear)
    Dim output, i, parts
    ...
    If UBound(arr) >= 0 Then
        For i = 0 To UBound(arr)
            parts = Split(arr(i), "|")
            output = output & "Record " & (i + 1) & ":" & vbCrLf
            output = output & "  Name: " & parts(0) & vbCrLf
            output = output & "  ID: " & parts(1) & vbCrLf
            If i < UBound(arr) Then output = output & vbCrLf
        Next
    Else
        output = output & "No records found." & vbCrLf
    End If
    
    ' Display in message box (truncated if too long)
    If Len(output) > 1024 Then
        output = Left(output, 1020) & "..." & vbCrLf & "(Output truncated)"
    End If
    
    LogMessage vbCrLf & output
End Sub


Sub LogMessage(message)
    On Error Resume Next
    fout.WriteLine Now & " - " & message
End Sub
...
Sub StartDoc()
    Dim xl, wb
    Dim branches, b, branch
    Dim thisYear, targetMonth

    thisYear = Year(Date)
    targetMonth = Month(Now) - 1  ' keep your prior behavior (through last month)

    ' maps variants LONGVIEW LOOP → LONGVIEW
    ' maps MT PLEASANT NORTH → MP NORTH
	
    Call NormalizeExistingBranchRows()
    ...
    ' Get all branches from DB (current year)
    branches = GetBranchesThisYear()
	
	' --- Debug: show branches returned ---
	Dim i, debugList, count

	If IsEmpty(branches) Then
		'MsgBox "GetBranchesThisYear() returned no branches."
		LogError "No branches found for current year."

	ElseIf IsArray(branches) Then
		count = UBound(branches) - LBound(branches) + 1
		debugList = ""
		For i = LBound(branches) To UBound(branches)
			debugList = debugList & CStr(branches(i)) & vbCrLf
		Next
		'MsgBox "Branches (" & count & "):" & vbCrLf & debugList
	Else
		fout.writeline  "Unexpected return type from GetBranchesThisYear(): " & TypeName(branches)
	End If
	' --- End debug ---
    ...
    For b = LBound(branches) To UBound(branches)
        branch = branches(b)
        ' Build a workbook for this branch from the single template
		'MsgBox "Building Workbook for " & branch & " month " & targetMonth
        Call BuildWorkbookForBranch(xl, branch, targetMonth)
    Next


End Sub

Sub BuildWorkbookForBranch(xl, branch, targetMonth)
    On Error Resume Next
    Dim wb, wsTemplate, ws
    Dim emps, i, empName, empID
    Dim m, colDate, mnth
    ...
    tempPath = GetTempCopyPath(branch)
    fso.CopyFile templatePath, tempPath, True

    Set wb = xl.Workbooks.Open(tempPath, False, False)
    If Err.Number <> 0 Then
        LogError "Failed to open temp copy: " & Err.Description
        Err.Clear
        Exit Sub
    End If
    On Error GoTo 0

    ' ensure totals is rightmost; we'll insert new sheets to its LEFT
    If wb.Sheets(TOTALS_SHEET_NAME).Index <> wb.Sheets.Count Then
        wb.Sheets(TOTALS_SHEET_NAME).Move Empty, wb.Sheets(wb.Sheets.Count) ' Move After:=last (VBScript positional)
    End If

    Set wsTemplate = wb.Sheets(TEMPLATE_SHEET_NAME)
    'wsTemplate.Visible = 0 ' hide template but keep as left boundary for formulas

    ' === get employees for this branch from DB (current year) ===
    emps = GetEmployeeNamesForBranchThisYear(branch)
	
	' --- Debug: list employees for this branch ---
	Dim j, empDebugList, empCount

	If IsEmpty(emps) Then
		'MsgBox "No employees found for branch: " & branch
		LogError "No employees found for branch: " & branch
	ElseIf IsArray(emps) Then
		empCount = UBound(emps) - LBound(emps) + 1
		empDebugList = ""
		For j = LBound(emps) To UBound(emps)
			empDebugList = empDebugList & CStr(emps(j)) & vbCrLf
			' also log each, if you want:
			fout.WriteLine branch & ": " & emps(j)
		Next
		'MsgBox "Employees for " & branch & " (" & empCount & "):" & vbCrLf & empDebugList
	Else
		'MsgBox "Unexpected return type from GetEmployeeNamesForBranchThisYear(): " & TypeName(emps)
		LogError "Unexpected return type from GetEmployeeNamesForBranchThisYear(): " & TypeName(emps)
	End If
	' --- End debug ---
    ...
    Else
        For i = LBound(emps) To UBound(emps)
			...
			' Insert a copy of the template immediately AFTER the active sheet
			wsTemplate.Copy , wb.Sheets("FSRTemplate")
			Set ws = wb.Sheets(wb.Sheets("FSRTemplate").Index + 1)
			 
			'MsgBox "Copied for: " & empName
			dim shtname
			'shtname = SafeSheetName(IIf(empID <> "", empName & " - " & empID, empName), wb)
	 
            ws.Name = empName & " - " & empID
		
            ' YTD fill: Jan -> targetMonth
            For m = 1 To targetMonth
                ...
                If Not rs Is Nothing Then WriteMonthlyTotalAccountsAssisted ws, rs, mnth

                ' Set rs = Fetch_FSR_DetailUserBranch(branch, empName, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyDetailUserBranch ws, rs, mnth

                ' Set rs = Fetch_FSR_NewLoanReportbyBranch(branch, empID, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyNewLoanReportbyBranch ws, rs, mnth

                ' Set rs = Fetch_FSR_SharesOpenedbyBranch(branch, empID, colDate)
                ' If Not rs Is Nothing Then WriteMonthlySharesOpenedbyBranch ws, rs, mnth

                ' Set rs = Fetch_FSR_ShareDFTSOpenedbyBranch(branch, empID, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyShareDFTSOpenedbyBranch ws, rs, mnth

                ' Set rs = Fetch_FSR_TimeDetailReport(branch, empName, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyTimeDetailReport ws, rs, mnth

                ' Set rs = Fetch_FSR_TellerActivity(branch, empID, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyTellerActivity ws, rs, mnth

                ' Set rs = Fetch_FSR_AcdCalls(branch, empName, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyAcdCalls ws, rs, mnth

                ' Set rs = Fetch_FSR_BranchMaintenance(branch, empName, colDate)
                ' If Not rs Is Nothing Then WriteMonthlyBranchMaintenance ws, rs, mnth
    ...
    ' Save a versioned copy in S:\Reports\
    'Call SaveWorkbookWithVersion(wb, outPath, "YTD")

    wb.Close True
    Set wb = Nothing

    ' clean up the temporary copy on disk
    On Error Resume Next
    If fso.FileExists(tempPath) Then fso.DeleteFile tempPath, True
    On Error GoTo 0

    fout.WriteLine "Workbook complete: " & fso.GetFileName(outPath) & vbCrLf
End Sub
...
Sub NormalizeExistingBranchRows()
    Dim c, tables, i, t
    Set c = OpenConnection()
    If c Is Nothing Then Exit Sub

    ' tables to normalize
	
	tables = Array( _
		"byREQUEST.dbo.FSR_ShareDFTSOpenedbyBranch", _
		"byREQUEST.dbo.FSR_NewLoanReportbyBranch", _
		"byREQUEST.dbo.FSR_TotalAccountsAssisted", _
		"byREQUEST.dbo.FSR_DetailUserBranch" _
	)
    ...
    For i = 0 To UBound(tables)
        ...
        ExecUpdate c, _
            "UPDATE " & t & " " & _
            "SET Branch = LTRIM(RTRIM(Branch)) " & _
            "WHERE Branch IS NOT NULL AND Branch <> LTRIM(RTRIM(Branch))", _
            "Trim spaces: " & t

        ' 2) Remove trailing ' Branch' (case-insensitive)
        ExecUpdate c, _
            "UPDATE " & t & " " & _
            "SET Branch = LEFT(LTRIM(RTRIM(Branch)), LEN(LTRIM(RTRIM(Branch))) - 7) " & _
            "WHERE Branch IS NOT NULL " & _
            "  AND RIGHT(UPPER(LTRIM(RTRIM(Branch))), 7) = ' BRANCH'", _
            "Drop suffix ' Branch': " & t

        ' 3) Map LONGVIEW LOOP -> LONGVIEW
        ExecUpdate c, _
            "UPDATE " & t & " " & _
            "SET Branch = 'LONGVIEW' " & _
            "WHERE Branch IS NOT NULL " & _
            "  AND UPPER(LTRIM(RTRIM(Branch))) = 'LONGVIEW LOOP'", _
            "Map LONGVIEW LOOP -> LONGVIEW: " & t

        ' 4) Map MT PLEASANT NORTH -> MP NORTH
        ExecUpdate c, _
            "UPDATE " & t & " " & _
            "SET Branch = 'MP NORTH' " & _
            "WHERE Branch IS NOT NULL " & _
            "  AND UPPER(LTRIM(RTRIM(Branch))) = 'MT PLEASANT NORTH'", _
            "Map MT PLEASANT NORTH -> MP NORTH: " & t
    Next
...
Sub ExecUpdate(c, sql, label)
    On Error Resume Next
    Dim n: n = 0
    c.Execute sql, n
    If Err.Number <> 0 Then
        LogError "Normalize error (" & label & "): " & Err.Description
        Err.Clear
    Else
        ' uses your existing fout logger; replace with WScript.Echo if needed
        'fout.WriteLine Now & " | INFO: " & label & " | rows affected=" & n
    End If
    On Error GoTo 0
End Sub
