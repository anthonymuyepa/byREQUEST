...
' Created: 
'   3-01-2017  Winship
'      Created for PIMA GL Accrual Detail for FASB 91
...
sub processline
	...
	If FileSummary <> 1 Then ' If we aren't in the summary section
		...
		If InStr(srec,"GL Accrual Detail for FASB 91") > 0 Then
		...
		Elseif InStr(srec,"Account    Name") > 0 AND FirstColumnHeader <> 1 Then
			Line2 = GetLine()
			NewLine = CalcMethod(0) & space(2) & srec & space(29 - len(srec)) & Line2
			FirstColumnHeader = 1
			fout.writeline NewLine
		...
		Elseif InStr(srec,"00") > 0 Then
			Line2 = GetLine()
			NewLine = CalcMethod(1) & space(20-len(CalcMethod(1))) & srec & space(29 - len(srec)) & Line2
			fout.writeline NewLine
	...
	If FileSummary = 1 Then
		...
		If InStr(srec,"GL Accrual Detail for FASB 91") Then
			SummaryHeaders = 1
