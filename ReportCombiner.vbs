...
Sub FlattenMergedDict(mergedDict)
    ...
    rowCount = 0
    ReDim allFlatRows(10000) ' You can expand later if needed
    ...
    For Each key In mergedDict.Keys
        ...
        ' Retrieve file-aligned rows
        For fileIndex = 1 To 4
            fileRows = mergedDict(key)(fileIndex - 1)
            If IsArray(fileRows) Then
                Set rowByFile(fileIndex) = CreateObject("Scripting.Dictionary")
                For j = 0 To UBound(fileRows)
                    rowByFile(fileIndex).Add j, fileRows(j)
                Next
            End If
        Next

        ' Build a flattened row using first row from each file
        flatRow = ""
        For i = 1 To 4
            If IsObject(rowByFile(i)) Then
                row = rowByFile(i).Item(0)
                flatRow = flatRow & Join(row, ",") & ","
            Else
                flatRow = flatRow & String(CountHeaderColumns(i), ",")
            End If
        Next

        ' Remove trailing comma
        If Right(flatRow, 1) = "," Then flatRow = Left(flatRow, Len(flatRow) - 1)

        flatFields = Split(flatRow, ",")
        If rowCount > UBound(allFlatRows) Then ReDim Preserve allFlatRows(rowCount + 1000)
        allFlatRows(rowCount) = flatFields
        rowCount = rowCount + 1
    Next

    ' Trim array size to actual row count
    ReDim Preserve allFlatRows(rowCount - 1)

    ' Recompute column widths based on ALL rows
    Dim colWidths
...
Function CountHeaderColumns(fileIndex)
    ...
    For i = 0 To UBound(parts)
        If Left(parts(i), 2) = "F" & fileIndex Then count = count + 1
    Next
